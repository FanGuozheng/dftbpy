...
# -*- coding: utf-8 -*-
import numpy as np
from math_torch import DFTBmath
import torch
...
def sk_tran(generalpara):
    ...
    hammat = torch.zeros((atomind2))
    overmat = torch.zeros((atomind2))
    rr = torch.zeros(3)
    for i in range(0, natom):
        ...
        for j in range(0, i+1):
            ...
            hams = torch.zeros((9, 9))
            ovrs = torch.zeros((9, 9))
...
def slkode(rr, i, j, generalpara, ham_matrix, s_matrix, lmax, hammat, overmat):
    ...
    dd = torch.sqrt(rr[0]*rr[0] + rr[1]*rr[1] + rr[2]*rr[2])
...
def getsk(generalpara, nameij, dd):
    ...
    if dd < grid0:
        ...
        datainterp = torch.zeros((int(ninterp), 20))
        ddinterp = torch.zeros(int(ninterp))
        ...
        datainterp[:, :] = torch.from_numpy(np.array(datalist[nlinesk-ninterp-1:nlinesk-1]))
