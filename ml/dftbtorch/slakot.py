                    ...
                    for n in range(0, atomind[j + 1] - atomind[j]):
                        ...
                        for m in range(0, atomind[i + 1] - atomind[i]):
                            ...
                            self.para['overmat_'][nn, mm] = \
                                self.para['ovrs'][n, m]'''
        # self.para['hammat'] = (self.para['ham_'] + self.para['ham_'].t()) / 2 + self.para['h_onsite'].diag()
        # self.para['overmat'] = (self.para['over_'] + self.para['over_'].t()) / 2 + self.para['s_onsite'].diag()
        self.para['hammat'] = self.para['ham_'] + self.para['h_onsite'].diag()
        self.para['overmat'] = self.para['over_'] + self.para['s_onsite'].diag()
        '''for ii in range(0, atomind[natom]):
            for jj in range(ii + 1, atomind[natom]):
                self.para['hammat'][ii, jj] = self.para['hammat'][jj, ii]
                self.para['overmat'][ii, jj] = self.para['overmat'][jj, ii]'''
        # print("self.para['overmat'] \n", self.para['overmat'], self.para['hammat'])
...
<<<<<<< HEAD
    ...
    def gen_psplpara(self):
        ...
        atomspecie = self.para['atomname_set']
        cutoff = self.para['interpcutoff']
        xp_start = self.para['grid0']
...
<<<<<<< HEAD
                ...
                if HNUM[nameij] > 0:
                    spl_ypara(self.para, nameij, griddist, xp_start, lines)
...
>>>>>>> ad0a72eac1ab68c13c8d6d1ed31874c22bb490c3
    ...
    if nhtable == 1:
        ...
        xx = t.linspace(1, 4, 151)
        ax.plot(xx, datalist_arr[49:200, 9], 'b-', lw=3, label='original')
        plt.show()
...
def shpar_(para, xyz, i, j, dd, li, lj):
    ...
    zz = xyz[2] / dd
    if lmax == 1:
        skss(xx, yy, zz, i, j, para['hs_all'], hams, ovrs, li, lj)
    elif lmin == 1 and lmax == 2:
...
def sksp_(xx, yy, zz, i, j, hs_all, ham, ovr, li, lj):
    ham, ovr = skss(xx, yy, zz, i, j, hs_all, ham, ovr, li, lj)
    if li == lj:
