# -*- coding: utf-8 -*-
"""
Created on Sun Feb 17 19:53:17 2019
...
general = {
    ...
    "tElec":0,
    "maxiter":60 }
valence_orbitals =  {"H": 1, "C": 2, "N": 5, "O": 6, "Ti": 3}
valence_electrons = {"H": 1, "C": 4, "N": 5, "O": 6, "Ti": 4}
atomName = [
...
class Main_Prog(object):
    ...
    def SCF(self):
        atom_ind=self.get_coor()[2]  #equal to sys%nOrbs
        maxiter=self.get_maxiter(general)
        scf=self.get_scf(general)
        num_atom=self.get_line_coordinate(general)[1]
        uHubb=self.get_electron()[0]
        qAtom=self.get_electron()[1]
        oldQAt=np.zeros((num_atom))
        ...
        for niter in range(0,maxiter):
            ...
            if scf=="T":
                for i in range(0,num_atom):
                    oldQAt[i] = qAtom[i]
            print("oldQAt,qAtom",oldQAt,qAtom)
            ...
            #print("before mulliken,oldQAt",oldQAt,qAtom)
            #calculate mulliken charges
            if scf:
                self.mulliken(num_atom,int(atom_ind[num_atom]),atom_ind,overMat,denMat,qAtom)
            #print("qAtom after mulliken",qAtom)
            #print("after mulliken,oldQAt",oldQAt)
            ...
            #for i in range(0,num_atom):
            #    qAtom[i] = oldQAt[i]
            mixFactor=self.get_mixFactor(general)
            broyden1=np.zeros((num_atom,2))
            broyden2=np.zeros((num_atom,2,maxiter))
            print("oldQAt",oldQAt,"qAtom",qAtom)
            if niter == 0:
                oldQAt=self.broyden0(niter,mixFactor,num_atom,broyden1,broyden2,oldQAt,qAtom)[0]
                F0=self.broyden0(niter,mixFactor,num_atom,broyden1,broyden2,oldQAt,qAtom)[1]
            else:
                F0=self.broyden1(niter,mixFactor,num_atom,broyden1,broyden2,oldQAt,qAtom,F0)[1]
            print("oldQAt",oldQAt,"qAtom",qAtom)
            for i in range(0,num_atom):
                qAtom[i] = oldQAt[i]
    
    # read input file #
    """the following code is to read the input file, including:
        read_in_file: read the input file
        get_line_coordinate: get which line the word coordinate is
        get_scf: whether scf is True or False
        get_task: what the task is, steady, lead or transport
        get_mixFactor: get the value of mixFactor
        get_maxiter: get the maxiter
        get_coor: read and generate the coordinate
        get_coor_vector: get and generate the vector between atoms"""
    ...
    # the following is read, and transform .skf file to H and S, 
    # and the corresponding operation od the H and S.
    """read_skf_file: read .skf file
       get_grid: return the value of gridDist, nGridPoints, skself
       get_line_Spline: get which the line of Spline is
       gMatrix: 
       gamsub
       shiftHamGamma
       fermi
    ...
    def gMatrix(self,uHubb):
        distance=self.get_coor_vector()[2]
        #print(distance)
        num_atom=self.get_line_coordinate(general)[1]
        gMat=[]
        k=0
    ...
    def SK_trans(self):
        atom_ind=self.get_coor()[2]
        atom_name_all=self.get_coor()[3]
        num_atom=self.get_line_coordinate(general)[1]
        distance=self.get_coor_vector()[0]
        distance_norm=self.get_coor_vector()[1]
    ...
    def broyden0(self,niter,alpha,jtop,unit31,unit32,vecin,vecout):
        vector = np.zeros((jtop,2))
        df = np.zeros((jtop))
        maxiter = self.get_maxiter(general)
        F = np.zeros((jtop))
        dumvi = np.zeros((jtop))
        qAtom0=self.get_electron()[1]
        ...
        for i in range(0,ivsiz):
            F[i]=vector[i,1]-vector[i,0]
            print("F(i) in 0 iter",F[i])
        for i in range(0,ivsiz):
            unit31[i,0]=F[i]
        for i in range(0,ivsiz):
        ...
        #!!!!!!!!!unit31 not equal
        for i in range(0,ivsiz):
            dumvi[i]= vector[i,0] + amix*F[i]
            #print("dumvi[i]",dumvi[i],vector[i,0],amix,F[i])
        for i in range(0,ivsiz):
            vecin[i]=dumvi[i]
        return vecin,F

    def broyden1(self,niter,alpha,jtop,unit31,unit32,vecin,vecout,F0):
        vector = np.zeros((jtop,2))
        df = np.zeros((jtop))
        ui = np.zeros((jtop))
        vti = np.zeros((jtop))
    ...
    def HS_s_s(self,atomi_HS_type,atomj_HS_type,x,y,z,HS_data):
        self.atomi_HS_type=self.get_coor()[0][0,atomi_HS_type]
        self.atomj_HS_type=self.get_coor()[0][0,atomj_HS_type]  #type of atom, for C, is 6
